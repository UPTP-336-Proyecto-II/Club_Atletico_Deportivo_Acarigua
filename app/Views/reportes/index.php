...
function openModalCat(type) {
    ...
    if (type === 'atletas') {
        ...
        // Listener para cambiar el ID de categoría en la URL
        form.action = '<?= e(url('/admin/reportes/categoria/')) ?>' + '/0';
        document.getElementById('cat-select').onchange = function() {
            form.action = '<?= e(url('/admin/reportes/categoria/')) ?>' + this.value;
        };
        ...
        form.action = '<?= e(url('/admin/reportes/asistencia/categoria')) ?>';
        document.getElementById('cat-select').onchange = null; // Envía categoría como query param
    }
