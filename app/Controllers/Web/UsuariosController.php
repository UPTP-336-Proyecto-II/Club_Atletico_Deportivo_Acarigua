...
final class UsuariosController extends Controller
{
    ...
    private function input(Request $request): array
    {
        return [
            'nombre'    => trim((string) $request->input('nombre')),
            'apellido'  => trim((string) $request->input('apellido')),
            'cedula'    => preg_replace('/[^A-Za-z0-9\-]/', '', strtoupper((string) $request->input('cedula'))),
            'telefono'  => trim((string) $request->input('telefono')),
    ...
    {
        $cedulaRule = 'required|min:6|max:20|regex:/^[VEP]-[A-Z0-9]+$/';
        if ($ignoreId) {
        ...
        $v = Validator::make($data, [
            ...
            'cedula'       => 'El campo número de documento de identidad debe tener un formato válido (Ej: V-12345678 o P-A1234567).',
            'parroquia_id' => 'El campo parroquia es obligatorio.',
